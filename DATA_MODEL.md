...
**Footer directory** (`lib/data/footer.ts`): `getFooterCities()` = priority-1, indexable cities by population (64), `getFooterCountries()` = published countries by population (40), `getFooterTimezones()` = `FOOTER_TIMEZONE_SLUGS` (18, curated). Tested in `lib/data/footer.test.ts`.
