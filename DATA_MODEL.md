...
```ts
type TimerPreset = {
  slug: string;        // canonical: timerSlugForSeconds(seconds) → "1-hour", "5-minutes"
  seconds: number;
  label: string;       // "1 Hour"
  phrase: string;      // "1 hour"
  chip: string;        // "1 hour" / "5 min"
  useCases: string[];  // duration-specific, not boilerplate
  quickPreset: boolean;// shown as a chip on timer pages
  priority: number;
  indexable: boolean;
};
```

Alias redirects (`60-minutes`, `1-hours`, `60-min`, …) are generated by `getTimerAliases()` and registered in `next.config.ts`. Tests guarantee no alias collides with a canonical slug.

**To add a timer page:** append a preset whose slug equals `timerSlugForSeconds(seconds)`.
