...
| End-to-end (Playwright, two production builds, desktop + mobile + kill switch) | 98 tests — **CI on `5e6210d`: 93 passed, 5 intentionally skipped, 0 failed**; local full runs showed single load-related timeouts that pass in isolation (§10.2) |
...
**Evidence (`npx playwright test`):** 98 tests. Full run on the final code: `91 passed, 5 skipped, 2 failed (3.9m)`; the two failures were the Sprint 1 mobile tests "search finds a city with the keyboard" and "mobile bottom navigation and menu work" (`toHaveURL` after 5 s, see §8 item 6). After raising their navigation timeout to 15 s: `npx playwright test --last-failed` → `2 passed (2.1m)`. A further full run with the hardened tests: `92 passed, 5 skipped, 1 failed (4.9m)` — this time the Sprint 1 "custom timer reaches zero" test hit its 30 s test timeout on this Windows machine; `--last-failed` → `1 passed (2.9m)`. **GitHub Actions on commit `5e6210d` (Linux, nothing else running): 98 tests, 93 passed, 5 skipped, 0 failed**, verify job 50 s, Playwright job 2 m 15 s. Every test in the suite passes; the two local timeouts are load effects of running three Next servers plus builds on the development machine. Skips are intentional: the bottom-navigation test is mobile-only and the four request-level checks (Sprint 1 HTTP, Sprint 2/3/4 routing) run once on desktop.
